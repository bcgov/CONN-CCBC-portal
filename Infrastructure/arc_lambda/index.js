...
exports.handler = async (event, context, callback) => {
  ...
  if (
    record.s3.object.key.indexOf('Intake_') > -1 &&
    record.s3.object.key.indexOf('json') > -1
  ) {
    ...
    if (list && Array.isArray(list)) {
      ...
      for (const record of list) {
        ...
        if (record.name.indexOf(INFECTED_FILE_PREFIX) > -1) {
          ...
          if (objectSrc && objectSrc.Body) {
            console.log(
              `downloaded ${record.name} from s3, key: ${record.uuid}`
            );
      ...
      if (issues.length > 0) {
        summary +=
          'Errors detected - please send this file to meherzad.romer@gov.bc.ca \r\n';
        summary += currentDateTime() + '\r\n\r\n';
        summary +=
          'These files were not included in the downloaded zip file due to errors:\r\n\r\n';
        issues.forEach((x) => {
      ...
      console.log(
        `uploaded ${key} to s3, size on disk: ${fileSizeInBytes}, blob size ${blob.length}`
      );
      return { key };
    }
  }
