...
begin
    select count(*) as total, 'new' as data_type,string_agg(ccbc_number, ', ') AS ccbc_numbers 
    ...
    where batch_id=batchId
        ...
        (select application_id from ccbc_public.application_gis_data where batch_id<>1 group by application_id)
    union all
    select count(*) as total, 'updated' as data_type,string_agg(ccbc_number, ', ') AS ccbc_numbers   
        from ccbc_public.application_gis_data agd
        inner join ccbc_public.application app on agd.application_id=app.id 
    where batch_id=batchId
    ...
    union all
        ...
        from ccbc_public.gis_data where id=1;
end;
