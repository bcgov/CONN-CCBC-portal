...
returns setof ccbc_public.history_item as $$
  ...
  union all
    ...
    from ccbc_public.record_version as v
        inner join ccbc_public.ccbc_user u on v.created_by=u.id
    where (v.op='INSERT' or v.op='UPDATE') and v.table_name='rfi_data' and v.record->>'archived_by' is null
  ...
  union all
   ...
   union all
    ...
    from ccbc_public.record_version as v
        inner join ccbc_public.ccbc_user u on v.created_by=u.id
    where v.op='INSERT' and v.table_name='application_milestone_data' and v.record->>'archived_by' is null and v.record->>'history_operation'='created'
        and v.record->>'application_id'=application.id::varchar(10)
