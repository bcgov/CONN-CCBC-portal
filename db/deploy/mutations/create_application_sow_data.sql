...
begin
  ...
  if exists (select * from ccbc_public.application_sow_data where id = old_application_sow_data_id)
    then update ccbc_public.application_sow_data set archived_at = now(), history_operation = _history_operation where id = old_application_sow_data_id and archived_at is null;
  end if;
 ...
 -- archive all the sow tabs related to the old sow id
  if exists (select * from ccbc_public.sow_tab_1 where sow_id = old_application_sow_data_id)
    then update ccbc_public.sow_tab_1 set archived_at = now() where sow_id = old_application_sow_data_id and archived_at is null;
  end if;

  if exists (select * from ccbc_public.sow_tab_2 where sow_id = old_application_sow_data_id)
    then update ccbc_public.sow_tab_2 set archived_at = now() where sow_id = old_application_sow_data_id and archived_at is null;
  end if;

  if exists (select * from ccbc_public.sow_tab_7 where sow_id = old_application_sow_data_id)
    then update ccbc_public.sow_tab_7 set archived_at = now() where sow_id = old_application_sow_data_id and archived_at is null;
  end if;

  if exists (select * from ccbc_public.sow_tab_8 where sow_id = old_application_sow_data_id)
    then update ccbc_public.sow_tab_8 set archived_at = now() where sow_id = old_application_sow_data_id and archived_at is null;
  end if;
