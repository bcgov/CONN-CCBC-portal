...
begin
  ...
  if exists (select * from ccbc_public.application_claims_data where id = _old_claims_id) then
    update ccbc_public.application_claims_data set archived_at = now() where id = _old_claims_id returning excel_data_id into old_excel_data_id;

    -- set history_operation to 'updated' if old claims data exists
    insert into ccbc_public.application_claims_data (application_id, json_data, excel_data_id, history_operation)
    values (_application_id, _json_data, _excel_data_id, 'updated')
    returning id into new_id;
  else
    -- history_operation defaults to 'created' if no old claims data exists
