...
create unique index ccbc_user_session_sub on ccbc_public.ccbc_user(session_sub);
...
perform ccbc_private.upsert_policy('ccbc_guest_select_ccbc_user', 'ccbc_user', 'select', 'ccbc_guest', 'session_sub=(select sub from ccbc_public.session())');
...
comment on column ccbc_public.ccbc_user.session_sub is 'Universally Unique ID for the user, defined by the single sign-on provider';
