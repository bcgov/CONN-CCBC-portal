-- Deploy ccbc:util_functions/upsert_timestamp_columns to pg
-- requires: trigger_function/update_timestamps

begin;

create or replace function ccbc_private.upsert_timestamp_columns(
...
begin
    ...
    if add_archive = true then
      ...
      if not exists (select *
        ...
        trigger_string := concat(
          ...
          ' for each row execute procedure ccbc_private.archived_records_are_immutable()'
        );
  ...
  if not exists (select *
    ...
    trigger_string := concat(
      ...
      ' for each row execute procedure ccbc_private.update_timestamps()'
    );
    execute(trigger_string);
  end if;
...
comment on function ccbc_private.upsert_timestamp_columns(text, text, boolean, boolean, boolean, text, text)
  is $$
