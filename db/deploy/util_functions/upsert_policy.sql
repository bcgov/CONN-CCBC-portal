-- Deploy ccbc:util_functions/upsert_policy to pg
...
-- example: select ccbc_private.upsert_policy('test_policy', 'test_table', 'update', 'admin', 'true');
-- becomes: create policy test_policy on ccbc.test_table for update to admin using(true) with check(true);
create or replace function ccbc_private.upsert_policy(
  policy_name text,
  table_name text,
  operation text,
  role_name text,
  using_check_statement text,
  table_schema_name text default 'ccbc_public'
)
returns void
as
$function$
...
-- example: select ccbc_private.upsert_policy('test_policy', 'test_table', 'update', 'admin', 'using(true)', 'with check(false)');
-- becomes: create policy test_policy on ccbc.test_table for update to admin using(true) with check(false);
create or replace function ccbc_private.upsert_policy(
  policy_name text,
  table_name text,
  operation text,
...
comment on function ccbc_private.upsert_policy(text, text, text, text, text, text) is
  'A generic function for creating or altering row-level security policies';
comment on function ccbc_private.upsert_policy(text, text, text, text, text, text, text) is
