-- Deploy ccbc:util_functions/verify_grants to pg
-- requires: schemas/public

begin;

-- Verifies permission grants for all columns in a table
create or replace function ccbc_private.verify_grant(
  operation text,
...
comment on function ccbc_private.verify_grant(text, text, text, text) is
...
comment on function ccbc_private.verify_grant(text, text, text, text[], text) is
