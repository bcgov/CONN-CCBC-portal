...
select has_function(
  'ccbc_private', 'verify_grant',
  'Function verify_grant should exist'
);

-- Passes when should pass

grant all on table ccbc_public.test_table to test_role;

select lives_ok(
  $$
    select ccbc_private.verify_grant('all', 'test_table', 'test_role');
  $$,
  'verify_grant passes when all permissions granted to user'
);

revoke all on table ccbc_public.test_table from test_role;
grant select on table ccbc_public.test_table to test_role;

select lives_ok(
  $$
    select ccbc_private.verify_grant('select', 'test_table', 'test_role');
  $$,
  'verify_grant passes when one permission granted to user'
);

revoke all on table ccbc_public.test_table from test_role;
grant all (allowed) on table ccbc_public.test_table to test_role;

select lives_ok(
  $$
    select ccbc_private.verify_grant('all', 'test_table', 'test_role', ARRAY['allowed']);
  $$,
  'verify_grant passes when all permissions granted to user on a specific column'
);

revoke all on table ccbc_public.test_table from test_role;
grant select (allowed) on table ccbc_public.test_table to test_role;

select lives_ok(
  $$
    select ccbc_private.verify_grant('select', 'test_table', 'test_role', ARRAY['allowed']);
  $$,
...
select throws_ok(
  $$
    select ccbc_private.verify_grant('bubbles', 'test_table', 'test_role');
  $$,
  'P0001',
