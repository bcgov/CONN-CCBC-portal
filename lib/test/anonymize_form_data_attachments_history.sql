...
WHERE table_name = 'form_data' AND (
  (record IS NOT NULL AND (record->'json_data' ? 'coverage' OR record->'json_data' ? 'templateUploads' OR record->'json_data' ? 'supportingDocuments')) OR
  (old_record IS NOT NULL AND (old_record->'json_data' ? 'coverage' OR old_record->'json_data' ? 'templateUploads' OR old_record->'json_data' ? 'supportingDocuments'))
);
